...
# "perm"     — step dummies with RAW SR coefficients (not LR-amplified)
build_yp <- function(lr, df, interp, raw = NULL) {
  switch(interp,
         ...
         perm   = {
           if (is.null(raw)) stop("perm interp requires raw dummy coefs")
           lr$a + lr$theta * df$k +
             raw$d1956 * as.integer(df$year >= 1956) +
             raw$d1974 * as.integer(df$year >= 1974) +
             raw$d1980 * as.integer(df$year >= 1980)
         },
...
# Raw SR dummy coefficients for Series F (step interpretation)
# Must be defined before u_F is built in Step 3
...
raw_sc <- get_raw_dummies(mod_shaikh_const)
cat(sprintf("F     (raw SR dums)  — d56: %.4f  d74: %.4f  d80: %.4f\n",
            raw_sc$d1956, raw_sc$d1974, raw_sc$d1980))
...
u_F <- build_u(df_plot$y, build_yp(lr_sc, df_plot, "perm", raw = raw_sc))  # corrected step
