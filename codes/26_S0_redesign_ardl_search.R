...
#
# Fix Case 3 (unrestricted intercept, no trend) — the only case
# consistent with Shaikh's capital-embodied technical change closure.
# Sweep ARDL(p,q) over p in {1..4}, q in {0..4} (20 specs).
# Dual admissibility gate: F-bounds AND t-bounds at 10% (exact=TRUE).
# Five IC criteria: AIC, BIC, HQ, ICOMP, RICOMP (sandwich).
#
...
cat("=== S0 REDESIGN — Case 3 Only, Exact Bounds, 5-IC Grid Search ===\n")
cat("Timestamp: ", now_stamp(), "\n", sep = "")
cat("Branch: S0_redesign | Case: 3 | exact=TRUE | Grid: ",
    length(P_RANGE), "x", length(Q_RANGE), "=",
    length(P_RANGE) * length(Q_RANGE), " specs\n\n", sep = "")
...
# Check corporate_sector_dataset.csv for GVA_nfc column
df_check <- readr::read_csv(here::here(CONFIG$data_shaikh), show_col_types = FALSE)
cat("\nColumns in corporate_sector_dataset.csv:\n")
...
# ============================================================
# STEP 4 — GRID SEARCH (20 SPECS, CASE 3 ONLY)
# ============================================================
cat(paste(rep("=", 60), collapse = ""), "\n")
cat("STEP 4 — GRID SEARCH\n")
cat(paste(rep("=", 60), collapse = ""), "\n\n")

grid <- expand.grid(p = P_RANGE, q = Q_RANGE, stringsAsFactors = FALSE)
cat("Grid size:", nrow(grid), "specifications\n\n")

# Formula: Case 3 (unrestricted intercept, no trend)
...
for (i in seq_len(nrow(grid))) {
  pp <- grid$p[i]
  qq <- grid$q[i]
  spec_label <- paste0("ARDL(", pp, ",", qq, ") Case 3")
  ...
  if (is.null(fit)) {
    results_list[[i]] <- data.frame(
      p = pp, q = qq, case = CASE_ID, spec = spec_label,
  ...
  # F-bounds
  bt_f_010 <- tryCatch(
    ARDL::bounds_f_test(fit, case = CASE_ID, alpha = 0.10, pvalue = TRUE, exact = EXACT_TEST),
    error = function(e) list(statistic = NA_real_, p.value = NA_real_))
  bt_f_005 <- tryCatch(
    ARDL::bounds_f_test(fit, case = CASE_ID, alpha = 0.05, pvalue = TRUE, exact = EXACT_TEST),
    error = function(e) list(statistic = NA_real_, p.value = NA_real_))
  bt_f_001 <- tryCatch(
    ARDL::bounds_f_test(fit, case = CASE_ID, alpha = 0.01, pvalue = TRUE, exact = EXACT_TEST),
    error = function(e) list(statistic = NA_real_, p.value = NA_real_))

  # t-bounds
  bt_t_010 <- tryCatch(
    ARDL::bounds_t_test(fit, case = CASE_ID, alpha = 0.10, pvalue = TRUE, exact = EXACT_TEST),
    error = function(e) list(statistic = NA_real_, p.value = NA_real_))
  bt_t_005 <- tryCatch(
    ARDL::bounds_t_test(fit, case = CASE_ID, alpha = 0.05, pvalue = TRUE, exact = EXACT_TEST),
    error = function(e) list(statistic = NA_real_, p.value = NA_real_))
  bt_t_001 <- tryCatch(
    ARDL::bounds_t_test(fit, case = CASE_ID, alpha = 0.01, pvalue = TRUE, exact = EXACT_TEST),
    error = function(e) list(statistic = NA_real_, p.value = NA_real_))
  ...
  # ----- Utilization -----
  if (!is.null(lr_full)) {
    u_series <- compute_u_from_lr(df, "lnY", "lnK", lr_full, dummy_names)
...
cat("Admissible specs (F + t both pass at 10%):", n_admissible, "of", nrow(results), "\n")

if (n_admissible == 0) {
  cat("\nWARNING: No specifications pass both admissibility gates.\n")
  cat("Cannot select IC winners. Reporting full grid and Shaikh benchmark only.\n\n")
}
...
if (n_admissible > 0) {
  for (ic in IC_NAMES) {
    ic_vals <- admissible_set[[ic]]
    if (all(is.na(ic_vals))) {
      cat("  ", ic, ": no valid values — skipped\n")
      next
    }
    best_idx <- which.min(ic_vals)
    winners[[ic]] <- admissible_set[best_idx, ]
    cat(sprintf("  winner_%s: ARDL(%d,%d) | %s=%.3f | theta=%.4f\n",
                ic, winners[[ic]]$p, winners[[ic]]$q,
                ic, winners[[ic]][[ic]],
                winners[[ic]]$theta))
  }

  # Check consensus
  winner_specs <- sapply(winners, function(w) paste0(w$p, ",", w$q))
  ...
  # Check Shaikh benchmark
  shaikh_in_admissible <- any(admissible_set$p == 2 & admissible_set$q == 4)
  cat("\nShaikh ARDL(2,4) in admissible set:", ifelse(shaikh_in_admissible, "YES", "NO"), "\n")

  # Check ICOMP vs RICOMP divergence
  if (!is.null(winners[["ICOMP"]]) && !is.null(winners[["RICOMP"]])) {
    icomp_spec <- paste0(winners[["ICOMP"]]$p, ",", winners[["ICOMP"]]$q)
    ricomp_spec <- paste0(winners[["RICOMP"]]$p, ",", winners[["RICOMP"]]$q)
    if (icomp_spec != ricomp_spec) {
      cat("\nICOMP and RICOMP winners DIFFER:\n")
...
results$IC_winner <- ""
if (length(winners) > 0) {
  for (ic in names(winners)) {
    w <- winners[[ic]]
    idx <- which(results$p == w$p & results$q == w$q)
    if (length(idx) == 1) {
...
results_sorted <- results |> arrange(AIC) |> mutate(Rank = row_number())
results_sorted <- results_sorted |>
  select(Rank, spec, AIC, BIC, HQ, ICOMP, RICOMP,
         F_stat, F_pval, F_sig, t_stat, t_pval, t_sig,
         admissible, IC_winner, everything())

# Print summary table
for (i in seq_len(nrow(results_sorted))) {
  r <- results_sorted[i, ]
...
if (n_admissible > 0) {
  safe_write_csv(admissible_set |> arrange(AIC),
...
# Deduplicate winners
if (length(winners) > 0) {
  winner_specs_dedup <- unique(sapply(winners, function(w) paste0(w$p, ",", w$q)))
  winner_rows_for_csv <- list()
  ...
  for (ws in winner_specs_dedup) {
    ...
    which_ics <- names(winners)[sapply(winners, function(w) w$p == wp & w$q == wq)]
    ic_label <- paste(which_ics, collapse = " / ")
    if (length(which_ics) >= 2) ic_label <- paste0(ic_label, " [CONSENSUS]")

    spec_label <- paste0("ARDL(", wp, ",", wq, ") Case 3")
    cat(paste(rep("-", 60), collapse = ""), "\n")
    cat("=== IC WINNER:", spec_label, "[", ic_label, "] ===\n")
    cat(paste(rep("-", 60), collapse = ""), "\n\n")

    # Re-fit for diagnostics
    fit_w <- ARDL::ardl(formula = fml, data = df_ts, order = c(wp, wq))
    ...
    for (alpha_lev in ALPHA_LEVELS) {
      ...
      bt_f <- tryCatch(
        ARDL::bounds_f_test(fit_w, case = CASE_ID, alpha = alpha_lev,
                            pvalue = TRUE, exact = EXACT_TEST),
        error = function(e) list(statistic = NA, p.value = NA))
      bt_t <- tryCatch(
        ARDL::bounds_t_test(fit_w, case = CASE_ID, alpha = alpha_lev,
                            pvalue = TRUE, exact = EXACT_TEST),
        error = function(e) list(statistic = NA, p.value = NA))
    ...
    # IC coordinates
    w_row <- results[results$p == wp & results$q == wq, ]
    cat(sprintf("\nIC coordinates:\n  AIC=%.1f | BIC=%.1f | HQ=%.1f | ICOMP=%.1f | RICOMP=%.1f\n",
                w_row$AIC, w_row$BIC, w_row$HQ, w_row$ICOMP, w_row$RICOMP))
    cat("\n")

    winner_rows_for_csv[[ws]] <- w_row |> mutate(IC_selected = ic_label)
  }

  # Save IC winners CSV
  ic_winners_csv <- bind_rows(winner_rows_for_csv)
...
shaikh_row <- results[results$p == 2 & results$q == 4, ]

if (nrow(shaikh_row) == 1) {
  cat("Specification: ARDL(2,4) Case 3\n\n")

  # Re-fit for full detail
  fit_shaikh <- ARDL::ardl(formula = fml, data = df_ts, order = c(2, 4))
  ...
  for (alpha_lev in ALPHA_LEVELS) {
    ...
    bt_f_s <- tryCatch(
      ARDL::bounds_f_test(fit_shaikh, case = CASE_ID, alpha = alpha_lev,
                          pvalue = TRUE, exact = EXACT_TEST),
      error = function(e) list(statistic = NA, p.value = NA))
    bt_t_s <- tryCatch(
      ARDL::bounds_t_test(fit_shaikh, case = CASE_ID, alpha = alpha_lev,
                          pvalue = TRUE, exact = EXACT_TEST),
      error = function(e) list(statistic = NA, p.value = NA))
  ...
  cat("WARNING: ARDL(2,4) not found in results grid.\n")
}
...
if (length(winners) > 0) {
  ...
  for (ws in winner_specs_dedup) {
    ...
    grid_idx <- which(grid$p == wp & grid$q == wq)
    ...
    p_fig <- ggplot(plot_long, aes(x = year, y = u, color = series, linetype = series)) +
      ...
      labs(
        ...
        title = paste0("Capacity utilization: ", spec_label, " [", ic_label, "] vs Shaikh vs FRB")
      )
