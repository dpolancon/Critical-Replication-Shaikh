############################################################
# 56_fetch_bea_extension.R — Extend corporate sector dataset
#
# Fetches current BEA/FRED data and runs GPIM recursion
# from the 1947 anchor to extend the sealed dataset to present.
#
# Output:  data/processed/corporate_sector_dataset_ext.csv
# NEVER overwrites: data/processed/corporate_sector_dataset.csv
#
...
## (Comments in 51_build_corp_output.R say 74/53, code uses 49/58)
...
## Imputed interest adjustment (replicating 51_build_corp_output.R logic)
df_nipa <- df_nipa |>
  mutate(
...
## This is consistent with the sealed pipeline (52_build_corp_kstock.R ADJ1=TRUE).
...
out_path <- here::here("data", "processed", "corporate_sector_dataset_ext.csv")
