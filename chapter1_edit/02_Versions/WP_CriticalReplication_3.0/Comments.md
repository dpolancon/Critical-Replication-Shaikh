...
**Current Content:**
> While Shaikh focuses his methodology on the error-correction term—using a residual that satisfies cointegration properties to establish a long-run relation between output and capital—this chapter reinterprets his methodology by focusing on the estimated coefficient itself and its implications for technological change, demand-led growth, and the political economy of growth and distribution. We interpret this parameter as a capacity transformation elasticity, $\theta$, which measures how capital accumulation translates into productive capacity. Standard post-Keynesian models typically assume a balanced-growth closure where this elasticity is constrained to unity ($\theta = 1$), meaning that Harrodian instability is triggered solely by aggregate demand deviations from normal capacity utilization. If $\theta \neq 1$, however, the path of accumulation and capacity formation becomes structurally unbalanced, defining either an overaccumulation regime ($\theta < 1$) or a regime of chronic excess capacity ($\theta > 1$). The central research inquiry of this chapter is not merely whether Shaikh's capacity index is reproducible, but whether the underlying transformation elasticity is unique across an admissible grid of single-equation models and whether the cointegration relation between output and capital is stable in a joint system.
