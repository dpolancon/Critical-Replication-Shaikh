...
\subsection{Laws of Algebra, Laws of Production, and Specification Challenges}
...
\bottomrule
\end{tabular}
\caption*{Note: Stage $S2$ introduces $\ln e_t$ (logged exploitation) to the trivariate state vector. Distributional variables enter only at the system-admissibility gate. Full series definitions, sources, and transformations are provided in Table~\ref{tab:app_glossary} of Appendix~\ref{app:data_construction}.}
\end{table}
...
\paragraph{4.22}
Over the replication period (1947--2011), the real corporate sector of the US economy exhibited an average annual output growth rate of approximately 3.2\%, while the real corporate capital stock grew at an average annual rate of 3.0\%, and aggregate employment grew at 1.5\%.%TODO: verify these growth rates against raw replication data (not AI-generated)
These raw rates indicate that corporate output and capital accumulated at similar but slightly divergent rates over the postwar period, generating a secularly shifting output--capital ratio. 
...
\label{subsec:cross_stage_synthesis_design}
...
\begin{table}[H]
\centering
\caption{Stage S0 Long-Run Estimation Parameters}
\label{tab:s0_model_parameters}
\footnotesize
\begin{tabular}{lcccccc}
\toprule
 & \multicolumn{3}{c}{Faithful Reconstruction: ARDL(2,4)} & \multicolumn{3}{c}{Focal Specification: ARDL(4,3)} \\
Parameter & Estimate & SE & $t$-stat & Estimate & SE & $t$-stat \\
\midrule
\midrule
$\hat{\theta}$ (elasticity) & 0.720 & 0.061 & 11.82$^{***}$ & 0.748 & 0.041 & 18.43$^{***}$ \\
$a$ (intercept) & 0.570 & 0.415 & 1.37 & 0.374 & 0.274 & 1.36 \\
$D_{1956}$ & -0.649 & 0.323 & -2.01$^{**}$ & -0.490 & 0.211 & -2.32$^{**}$ \\
$D_{1974}$ & -0.841 & 0.386 & -2.18$^{**}$ & -0.637 & 0.246 & -2.59$^{**}$ \\
$D_{1980}$ & -0.456 & 0.226 & -2.02$^{**}$ & -0.361 & 0.154 & -2.34$^{**}$ \\
\midrule
RSE & \multicolumn{3}{c}{0.0167} & \multicolumn{3}{c}{0.0164} \\
$1 - \sum \gamma_i$ (denominator) & \multicolumn{3}{c}{0.096} & \multicolumn{3}{c}{0.127} \\
\bottomrule
\end{tabular}
\caption*{Note: Standard errors for $\hat{\theta}$ and the dummy variables are estimated using the delta method. Significance: $^{***}$ $p<0.01$, $^{**}$ $p<0.05$. Source: Author's elaboration.}
\end{table}
...
\begin{enumerate}
    \item The **shared AIC/BIC/HQ/ICOMP winner: $\text{ARDL}(3,3)$, Case 1 (no-constant/no-trend), with the $s_1$ dummy (1974)**, which yields $\hat{\theta} = 0.92$ (SE 0.04).
    \item The **RICOMP winner: $\text{ARDL}(1,2)$, Case 2 (restricted constant), with no dummies ($s_0$)**, which yields $\hat{\theta} = 0.65$ (SE 0.07).
\end{enumerate}
...
\begin{table}[H]
\centering
\caption{Stage S2 Focal VECM Parameter Estimates}
\label{tab:s2_focal_vecm}
\footnotesize
\begin{tabular}{lccc}
\toprule
 & Cointegrating Vector ($\beta$) & Loading Vector ($\alpha$) & Speed of Adjustment \\
Variable & Coefficient & Loading Coeff. & $t$-statistic \\
\midrule
\midrule
$\ln Y_t$ (Output) & 1.000 & -0.005$^{**}$ & -2.00 \\
$\ln K_t$ (Capital) & -0.727 (SE 4.852) & 0.000 & 0.92 \\
$\ln e_t$ (Exploitation) & 20.022$^{***}$ (SE 2.536) & -0.019$^{***}$ & -4.25 \\
\bottomrule
\end{tabular}
\caption*{Note: Cointegrating vector is normalized on $\ln Y_t$. Standard errors are in parentheses. Significance: $^{***}$ $p<0.01$, $^{**}$ $p<0.05$. Source: Author's elaboration.}
\end{table}

\paragraph{4.75}
The estimated cointegrating vector normalized on output yields:
\begin{equation}
\ln Y_t - 0.727 \ln K_t + 20.022 \ln e_t - \text{constant} \sim I(0)
\end{equation}
The capital elasticity $\hat{\theta} = 0.727$ aligns with the single-equation estimates. However, the high standard error ($4.852$) reflects the multi-collinearity introduced by the rate of exploitation. The coefficient on the rate of exploitation is highly significant ($\hat{\beta}_e = 20.022$, SE 2.536, t-stat $7.90$), indicating a strong level relationship between capacity levels and distribution. 
...
\begin{table}[H]
\centering
\caption{Focal VECM Short-Run Dynamics and Dummies}
\label{tab:s2_short_run_dynamics}
\footnotesize
\begin{tabular}{lccc}
\toprule
Regressor / Dummy & $\Delta \ln Y_t$ Equation & $\Delta \ln K_t$ Equation & $\Delta \ln e_t$ Equation \\
\midrule
\midrule
...
\midrule
$D_{1956}$ (dummy) & -0.005 (0.015) & 0.002 (0.002) & -0.062$^{**}$ (0.029) \\
$D_{1974}$ (dummy) & -0.038$^{**}$ (0.017) & 0.001 (0.002) & -0.078$^{**}$ (0.032) \\
$D_{1980}$ (dummy) & 0.001 (0.016) & -0.003 (0.002) & -0.014 (0.031) \\
\bottomrule
\end{tabular}
\caption*{Note: Standard errors are in parentheses. Significance: $^{***}$ $p<0.01$, $^{**}$ $p<0.05$. Source: Author's elaboration.}
\end{table}
