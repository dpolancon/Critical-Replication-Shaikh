...
\begin{abstract}
Capacity utilization depends on a productive ceiling that is never observed directly. \citet{Shaikh2016} offers a portable accounting-based solution by deriving productive capacity from a long-run output--capital relation. This chapter critically replicates the coefficient that draws that capacity path, not only the utilization series built from it. Once reconstructed, the empirical output--capital coefficient is interpreted here as a candidate transformation elasticity, $\theta$, linking capital accumulation to productive-capacity formation. Stage S0 finds that a Shaikh-like baseline is approximately recoverable from corporate national accounts, conditional on data-construction and specification choices. Stage S1 shows that a 500-model single-equation ARDL space does not collapse to a unique benchmark; some statistically retained alternatives also fail economic admissibility. Stage S2 moves the relation into a joint VECM system: the bivariate output--capital system is not self-sufficient, while a restricted trivariate system survives under the retained criteria when distribution and historical shock controls enter the long-run system. Shaikh's method is not discarded, but its structural interpretation requires explicit distributional and historical conditioning.
\end{abstract}
...
Capacity utilization begins from a measurement problem: actual output is visible, but the productive ceiling against which it is judged is not. \citet{Shaikh2016} offers a portable accounting-based solution by constructing productive capacity from a long-run output--capital relation, rather than from survey utilization rates or official output-gap filters.

The narrow replication target is the coefficient that generates Shaikh's productive-capacity path. Recovering the utilization series is not enough, because the series inherits its level, timing, and persistence from the long-run output--capital coefficient that determines the denominator of utilization.

The recovered empirical coefficient, $d$, then carries a stronger interpretation only after the theoretical mapping is stated. I read $\hat{d}$ as an estimated candidate transformation elasticity, $\hat{\theta}$, linked to the theoretical elasticity $\theta$ that connects capital accumulation to productive-capacity formation. That interpretive move is evaluated rather than assumed.

The empirical design follows the pressure points implied by that interpretation. Stage S0 reconstructs the closest recoverable Shaikh-like baseline; Stage S1 asks whether the coefficient is unique across the single-equation ARDL specification space; and Stage S2 tests whether the output--capital relation remains system-admissible when estimated in a joint VECM system.

Shaikh's baseline capacity path is approximately recoverable, but the coefficient that generates it is not structurally self-sufficient. It is sensitive across statistically retained single-equation choices, and the bivariate output--capital relation fails the retained system-admissibility gates. Under the retained S2 criteria, a restricted trivariate system survives when the logged rate of exploitation and historical shock controls enter the long-run system. Some statistically retained estimates fail economic admissibility and are not treated as valid capacity benchmarks.

Shaikh's approach is therefore narrowed rather than discarded. Its portability depends on making explicit the distributional and historical conditions under which the output--capital relation is stable. The chapter first locates the measurement problem historically, then defines the coefficient and its transformation-elasticity interpretation before turning to the three-stage replication and its bounded implications for profitability and capacity measurement.

\section{A Historical Trace of Capacity Utilization Measurement}
\label{sec:literature_review}

Capacity utilization entered macroeconomic measurement as a practical response to unobserved slack. In the postwar and Fordist policy setting, it translated idle plant and equipment into a language of demand management; after the inflationary crises of the 1970s, the same measure increasingly served as a signal of bottlenecks and inflationary pressure. Political economy later returned to the concept because neither survey utilization nor output-gap estimates resolved the deeper question of what determines the productive ceiling. The survey tradition is the starting point for that history.
...
\qquad d \equiv 1+c,
\end{equation}
Within this regression, $y_t$ and $k_t$ represent the natural logarithms of unscaled output and capital, while $d$ represents the empirical long-run output--capital coefficient recovered from the level relation. The constant $a$ absorbs initial conditions and fixed measurement discrepancies in the levels of output and capital. The coefficient $c$ governs how the normal capacity--capital ratio varies with capital, and the linear trend component ($bt$) is conventionally interpreted as a deterministic trend or autonomous growth component. If the underlying long-run behavior is instead driven by a proportional growth relation ($g_{Y^p} = \theta g_K$), this level trend functions as an algebraic adjustment factor that absorbs the scale differences between the unscaled time series.
...
The estimated long-run coefficient $d$ becomes a candidate transformation elasticity only after it is mapped onto the theoretical object $\theta \equiv \partial \ln Y^p / \partial \ln K$. Because the normal rate of profit relies directly on the constructed capacity--capital ratio, the structural integrity of the accumulation theory depends on whether this empirical coefficient can sustain that interpretation. While the aggregate accounting identity deterministically organizes these profit magnitudes, it leaves the underlying behavioral mechanism translating capital accumulation into productive capacity undefined. Following the analytical standard of \citet{Shaikh1974}, an accounting identity that leaves its core behavioral mechanism undefined mechanically mimics a structural production law, exposing the empirical framework to severe specification errors.
...
The empirical test now turns to the object defined in Section~\ref{sec:conceptual_framework}: Shaikh's long-run output--capital coefficient, which draws the productive-capacity path and is interpreted here as a candidate transformation elasticity only after reconstruction. The question is whether that relation itself, not only the utilization proxy derived from it, remains stable across admissible specifications and joint system estimation.

The three stages translate this object into empirical tests. S0 reconstructs the closest recoverable version of Shaikh's single-equation procedure. S1 opens the ARDL specification grid and separates statistical admissibility from economic admissibility. S2 moves from conditional single-equation estimation to joint VECM estimation, asking whether the bivariate output--capital relation is self-sufficient or survives under the retained criteria only when logged exploitation and historical shock controls enter the trivariate state vector.

The results narrow the conditions under which Shaikh's strategy can carry a structural interpretation. The single-equation coefficient is recoverable, but it does not replicate the original specification exactly and is non-unique within the screened ARDL space. In joint-system estimation, the bivariate relation fails the retained system-admissibility gates; a restricted trivariate system survives under the retained criteria with distributional conditioning. The evidence therefore locates the boundary conditions of Shaikh's portability strategy rather than discarding it.
...
Equation~\eqref{eq:ardl_uecm} writes the ARDL specification in unrestricted error-correction form, where $c_0$ represents the intercept and $c_1t$ the linear trend. The short-run dynamics are represented by $\Delta y_{t-i}$ and $\Delta k_{t-j}$, which capture the short-run memory of the model. The lagged level terms $y_{t-1}$ and $k_{t-1}$ carry the long-run content of the specification. The year-dummy block represents deterministic components that are excluded from the long-run relation $\sum_{h=1}^{H}\delta_h D_{h,t}$. This representation therefore separates short-run adjustment from the level relation between output and capital.
...
In Shaikh's procedure, this long-run multiplier anchors the fitted capacity path. Thus, capacity utilization is constructed as the residual deviation from the long-run cointegration vector: $\log \hat{\mu}_t = y_t - \hat{y}^p_t$. This sequence establishes a strict dependency: utilization derives entirely from the estimated coefficient. Shaikh treats $\hat{d}$ as a fitted accounting multiplier; he does not theorize it as a structural elasticity. This replication interprets $\hat{d}$ as the estimated candidate transformation elasticity $\hat{\theta}$ only after the theoretical mapping from capital accumulation to productive-capacity formation has been stated. Given the five ARDL deterministic cases and the year dummies used by Shaikh to control for historical outliers, the replication treats researcher decisions over specification grids, deterministic treatment, and admissibility gates as sources of variation. It stress-tests this assigned interpretation across three stages: reproducibility ($S0$), admissible-specification analysis ($S1$), and system cointegration ($S2$). Subsection~\ref{subsec:data_measurement} details the data construction and measurement conventions required to execute these stages.
...
Corporate gross value added is corrected for imputed financial intermediation services by subtracting net monetary interest paid and adding back imputed net interest adjustments (NIPA Table 7.11). The algebraic derivation and NIPA account usage are detailed in Appendix~\ref{subsec:app_impint}. Year dummies identified by Shaikh as residual spikes enter as deterministic controls $D_{56}, D_{74}, D_{80}$ in the ARDL baseline, following squared-error diagnostics \citep{Patterson2000}. They are treated here as historically interpretable shock controls associated with 1956, 1974, and 1980, not as standalone causal explanations of those episodes. The sample size ($T \approx 65$) is small for time-series inference and can bias cointegration inference when relying on asymptotic critical value bounds; the replication addresses this by computing exact-sample bounds calibrated to $T=65$ via stochastic simulation \citep{NatsiopoulosTzeremes2022}.

These measurement conventions constitute the identification strategy. For the capacity question, the relevant capital stock is the gross stock of capital in operation, not the net book-value stock used more commonly in profitability accounting. The GPIM reconstruction is appropriate here because it tracks the accumulation and depletion of operating capital through $z_t^*$, preserving the stock--flow relation needed to estimate productive capacity. Net capital stock may be more directly connected to balance-sheet valuation and profitability measurement, but the capacity baseline requires a measure of the physical stock available for production.\footnote{\citet{Shaikh2016} also argues that gross capital stocks are preferable for profitability indices. This chapter uses the narrower claim that gross GPIM-adjusted capital is appropriate for productive-capacity measurement.} Shaikh also argues for a common deflator, which isolates real capital--capacity dynamics from price-distortion effects.
...
We define "admissibility" as a sequence of filters rather than a single verdict. Statistical admissibility means that a specification passes the relevant econometric gate, such as ARDL bounds evidence or VECM rank and stability checks. Economic admissibility is narrower: the retained coefficient must produce an interpretable capacity parameter and a plausible utilization path. System admissibility refers specifically to survival under the joint VECM rank and companion-stability gates. Object admissibility asks whether the retained relation still estimates the intended capacity--capital mapping rather than a neighboring deterministic trend or shock-control object. A specification can therefore be statistically retained while failing economic admissibility.
...
\begin{equation}
\mathcal{G}_{S1} = \{(p,q,\chi,s)\},
\label{eq:G_S1}
\end{equation}
where $p$ and $q$ represent the ARDL lag orders, $\chi \in \{1,\dots,5\}$ indexes the deterministic configurations, and $s$ indexes the nested baseline outlier structures.
...
Identification fragility & Deterministic cases \& dummy structures & Track admissibility shifts across $\chi \in \{1,\dots,5\}$ and $s \in \{s_0,\dots,s_3\}$ & Sensitivity to deterministic components and structural trend assumptions \\
Information complexity \& Pareto optimization & Fit-complexity envelope \& IC neighborhoods & Pareto non-dominance in $E_{S1}$; 20th percentile cutoff by information criterion & Organization of disciplined non-uniqueness under distinct information penalty schedules \\
...
Section~\ref{subsec:S2_system} extends the critique to a joint multi-equation system. Moving beyond single-equation constraints, the system-level analysis tests whether bivariate cointegration fails the system-admissibility gates in a pattern consistent with omitted distributional conditioning. This system-level non-self-sufficiency determines whether joint system survival requires the logged rate of exploitation ($e_t$) to enter the long-run cointegrating space.

\subsubsection{Stage 2 \texorpdfstring{$(S2)$}: System Co-Integration Test}
\label{subsec:S2_system}

Stage S2 evaluates whether the bilateral output--capital relation remains self-sufficient when shifted from a conditional single-equation setup to a joint multi-equation system. The baseline bivariate vector, $X_t = (\ln Y_t, \ln K_t)'$, fails the retained Johansen rank conditions and companion-matrix stability gates. To test this system-level non-self-sufficiency, the expanded framework introduces explicit distributional conditioning by integrating the logged rate of exploitation, $e_t = \ln(\pi_t / (1 - \pi_t))$, yielding the trivariate state vector $X_t = (\ln Y_t, \ln K_t, \ln e_t)'$. This trivariate system is estimated using the full \citet{Johansen1991} Vector Error Correction Model (VECM) representation:
\begin{equation}
\label{eq:vecm_johansen}
\Delta X_t = \Pi X_{t-1} + \sum_{i=1}^{p-1} \Gamma_i \Delta X_{t-i} + \Phi D_t + \varepsilon_t,
\end{equation}
where $\Gamma_i$ captures short-run dynamics, $\Phi D_t$ contains deterministic controls, and the long-run impact matrix is factored as $\Pi = \alpha \beta'$ to evaluate reduced-rank stability. Within the retained S2 grid, joint system survival is retained only under this formulation, where the rank condition $r=1$ holds and the exploitation variable anchors the long-run cointegrating space ($\beta$).
...
\begin{equation}
\mathcal{G}_{S2}^{(r)} = \{(p, \chi, s)\},
\label{eq:G_S2}
\end{equation}
where $p \in \{1, 2, 3, 4\}$ represents the VAR lag orders, $\chi \in \{d_0, d_1, d_2, d_3\}$ indexes the Johansen deterministic closure labels, and $s \in \{h_0, h_1, h_2\}$ isolates a subset of the baseline historical outlier controls. The labels $d_0,\ldots,d_3$ denote deterministic VECM branches, not the long-run coefficient $d$ in equation~\eqref{eq:shaikh_regression}. Permuting across these state-vectors yields an attempted grid of 48 specifications per rank. However, because specifications under the restricted constant branch ($d_1$) systematically fail numerical convergence across all permutations, the successfully estimated baseline is analytically restricted to exactly 36 models per system family.
...
S2 Trivariate & Joint $(\ln Y_t, \ln K_t, \ln e_t)'$ VECM & Same as bivariate, with logged rate of exploitation included; $r=1$ or $r=2$ & Same triple gate; $\Omega_{20}$ organizes admissible survivors by log-likelihood loss & Tests whether the structural relation survives only within the retained S2 grid when distribution enters explicitly \\
\bottomrule
\end{tabular}
...
The published reference value is $\hat{\theta}=0.66$, whereas the closest recoverable reconstruction yields $\hat{\theta}=0.72$ from an ARDL(2,4) specification, which also emerges as the BIC winner in the S0 rerun. An alternative ARDL(4,3) specification, selected by the AIC, yields $\hat{\theta}=0.75$. The precise parameters for these two estimated S0 long-run multipliers are reported in Table~\ref{tab:s0_model_lr_summary}. The discrepancy between 0.66 and 0.72 is the primary S0 result: the baseline is approximately recoverable, but not exactly reproducible under documented choices. To recover the benchmark, an active search through Shaikh's raw spreadsheet archives was necessary to identify the undocumented deflator choice among competing price deflator indices, such as the BEA historical-cost series vs. Census book values.
...
Stage S0 confirms that a Shaikh-like capacity benchmark is approximately recoverable. However, recovering a single point estimate does not guarantee that the underlying structural parameter is unique or robust. To expose potential identification fragility, Stage S1 expands the estimation framework. This next stage tests whether the recovered benchmark remains stable across a wide grid of statistically retained single-equation alternatives.

\subsection{Stage 1 \texorpdfstring{$(S1)$}: Admissible-Specification Stress Test Results}
\label{subsec:S1_ardl_grid}

Stage S1 tests the model's stability by running a 500-regression grid search, systematically varying lag order, deterministic case, dummy structure, and information criteria to evaluate whether the baseline benchmark remains stable. In the notation of Section~\ref{subsec:empirical_design}, we move from a single ARDL point estimate to the fully screened grid space $\mathcal{G}_{S1}$. 

The resulting grid does not collapse around a single parameter, instead containing 500 successful combinatorial specifications. Applying the initial 10 percent bounds threshold, 102 specifications pass the primary $F$-bounds gate and enter the statistically retained $F$-admissible set $\mathcal{A}^{F}_{S1}$. As the asymptotic significance size tightens, the retained set predictably shrinks: 62 specifications remain $F$-admissible at the 5 percent threshold, and just 13 at the 1 percent limit. Unpacking this set reveals that cointegration is concentrated in specific cases and dummy configurations, with no-trend specifications dominating 90.2\% of the statistically retained space (Cases 1, 2, and 3) and trend-containing models representing only 9.8\%. Level stability is also highly conditional on dummy variables: 92.2\% of the $F$-admissible models require at least one structural break dummy variable ($s_1, s_2, s_3$) to pass the bounds test, indicating that step-shift dummies act as historically interpretable shock controls associated with 1956, 1974, and 1980. 
...
This strong statistical dependence on dummy variables aligns with the historical trajectory of the postwar US economy. The shock years correspond to major systemic disruptions: the 1956 Eisenhower recession, the 1974 first OPEC oil crisis and stagflation, and the 1980 Volcker monetary shock. In a pure bivariate level relation without outlier controls ($s_0$), these historical episodes appear as persistent, non-mean-reverting shocks in the residuals. In those specifications, the error term behaves as a random walk, violating stationarity and causing the bounds test to fail. Retained bounds evidence is obtained when step-shift dummy variables ($s_1, s_2, s_3$) absorb level shifts associated with these historical shocks and allow the error term to return to a zero-mean stationary path. The negative coefficients on these dummies are interpreted here as level adjustments in capital productivity ($Y/K$) associated with these crises. The dummies are therefore historically interpretable shock controls, not independent evidence of the full causal mechanism behind each episode.
...
The shared AIC/BIC/HQ/ICOMP winner is an ARDL(3,3), Case 1 specification with the 1974 historical dummy (\(s_1\)), yielding \(\hat{\theta}=0.92\). Conversely, RICOMP selects a highly parsimonious ARDL(1,2), Case 2 specification with no impulse dummies (\(s_0\)), yielding \(\hat{\theta}=0.65\). This criteria divergence does not establish a structural ranking of the underlying estimators. Unlike standard scalar penalties, the ICOMP and RICOMP criteria evaluate complexity through the covariance structure of estimated parameters and residual dependence \citep{Bozdogan1990,Bozdogan2000}. The RICOMP neighborhood therefore marks a covariance-complexity-sensitive region of the admissible space rather than providing a definitive model-selection verdict \citep{GuneyBozdoganArslan2021}. This parameter variation also matters for economic interpretation. While no-trend specifications (Cases 1, 2, 3) yield stable long-run elasticities bounded below unity ($\hat{\theta} \in [0.65, 0.95]$) that support the overaccumulation hypothesis, the trend-containing configurations (Cases 4, 5) yield economically inadmissible capacity-benchmark magnitudes ($\hat{\theta}$ averaging 1.36 and reaching up to 2.20). An elasticity $\theta > 1.0$ implies that productive capacity expands exponentially relative to capital accumulation in the long run, violating the classical overaccumulation baseline and indicating that statistical bounds-passing does not guarantee economic admissibility.
...
To evaluate this single-equation parameter sensitivity, consider a key pedagogical counterfactual: the restricted balanced-growth model where the transformation elasticity is constrained to unity ($\theta = 1.0$). If a researcher forces this standard Harrodian assumption onto the US dataset, the capacity path is forced to expand exactly in step with the capital stock (imposing a constant capital productivity baseline). When we construct capacity utilization under this constraint, the resulting series drifts to economically implausible levels, either falling continuously toward zero in the long run or exceeding 120\% during periods of rapid expansion. This occurs because the economically admissible estimates imply an unbalanced US output--capital relation ($\theta < 1.0$), meaning capacity grows more slowly than the capital stock. Imposing the $\theta=1$ counterfactual falls outside the retained economic-admissibility interpretation, showing why estimating the unrestricted parameter is not just a technical choice, but a requirement for economic viability.
...
Stage S1 demonstrates that the statistically retained single-equation space is populated, yet it does not support the assumption of a unique structural benchmark. Because the retained alternatives fail to collapse into a single transformation elasticity ($\hat{\theta}$), the resulting single-equation utilization path remains structurally fragile. Section~\ref{subsec:S2_system} addresses this fragility by testing whether the output--capital relation survives multi-equation system estimation, and whether survival under the retained criteria depends on distributional variables entering the state vector.

\subsection{Stage 2 \texorpdfstring{$(S2)$}: System-Level VECM Replication}
\label{subsec:S2_vecm}

Stage S2 extends the empirical analysis to a system-level vector error correction model (VECM) to evaluate the joint dynamics of output and capital. The bivariate state vector, $X_t=(\ln Y_t,\ln K_t)'$, is examined first. However, the bivariate system does not pass the retained system-admissibility gates across all estimated bivariate specifications. The VECM residuals remain nonstationary, indicating that the long-run relation between output and capital is not system-admissible when estimated in isolation.

To address this issue, the trivariate state vector $X_t=(\ln Y_t,\ln K_t,\ln e_t)'$ is introduced, where $\ln e_t$ represents the logged rate of exploitation, constructed as the aggregate corporate profit-share-to-wage-share ratio, $e_t=\pi_t/(1-\pi_t)$. The inclusion of $\ln e_t$ allows the test to place distributional variables within the cointegrating space. The system-level estimation is governed by a triple confirmation gate, consisting of numerical convergence, Johansen reduced-rank verification, and companion-matrix dynamic stability, to isolate retained long-run systems from unstable vector autoregressive processes.

The results are presented in Table~\ref{tab:s2_admissibility_outcomes}, which details the structural outcomes across the complete VECM specification space ($\mathcal{G}_{S2}$). The bivariate $r=1$ configuration yields zero system-admissible models, with VECM residuals that fail unit-root tests and remain nonstationary. In contrast, the trivariate $r=1$ framework yields six statistically retained system specifications when conditioned on the logged rate of exploitation, $\ln e_t$. The trivariate rank-two permutations confirm this restriction, producing zero stable dual-vector systems. This indicates that cointegration is retained only within the rank-one trivariate system in the estimated S2 grid, alongside step-shift dummies. The six retained systems must then pass a second economic-admissibility screen before they can be treated as valid capacity benchmarks.
...
The $h_2$ dummy vector plays a central role in the trivariate VECM models, as it accounts for the step shifts in 1956, 1974, and 1980, and it appears in each retained system-admissible specification. The uniform survival pattern of models that include this vector indicates that the single-equation outlier controls capture level shifts associated with system-level stability. Cointegration is retained only when these historically interpretable shock controls prevent persistent level shifts in the residual mean from appearing as unit roots in Johansen trace and Dickey-Fuller tests; the dummies do not by themselves identify the causal mechanisms behind each historical episode. Under the $d_3$ trend branch, which includes a linear time trend in the VECM cointegrating space, the models yield economically inadmissible parameter estimates, including explosive ($\hat{\theta} = 11.31$) or negative ($\hat{\theta} = -0.81$) elasticities, because the time trend absorbs the level drift of output, forcing the capital-output coefficient to capture short-run noise and structural breaks. These $d_3$ configurations are therefore economically rejected as capacity benchmarks. The $p2\_d0\_h2\_r1$ specification is also statistically retained but economically rejected as a capacity benchmark because its $\hat{\theta}=1.19$ lies above unity, reversing the chapter's economically interpretable overaccumulation closure. The economically admissible S2 benchmarks are therefore restricted to the positive below-unity estimates $\hat{\theta}=0.73, 0.91, 0.97$. The negative coefficients on the shock controls are interpreted as level shifts associated with 1956, 1974, and 1980, not as proof of a complete historical mechanism. The logged rate of exploitation $e_t$ is also associated with greater system-level stability. By incorporating the distribution variable into the state vector, the retained S2 design is consistent with a Sraffa-style choice-of-technique channel in which institutional distribution conditions the stable long-run output--capital relationship \citep{Kurz1986}.

The no-dummy counterfactual specification provides a key test of the importance of historical controls. Omitting the $h_2$ dummy vector, which controls for the 1956, 1974, and 1980 shocks, results in a VECM that fails to reject the null hypothesis of no cointegration, according to the Johansen trace and maximum eigenvalue tests. Moreover, the resulting VECM residuals are highly nonstationary, failing standard unit-root tests. This indicates that the estimated relationship does not pass the retained system-admissibility gates without an error-correction mechanism to pull output, capital, and exploitation back together. The counterfactual indicates that the system-level output--capital relationship is not retained when specific historical and institutional shock vectors are excluded. Under the retained S2 design, the cointegrating space is therefore conditional on these historical dummy variables rather than a permanent, institution-free law.
...
\begin{tabular}{lcccc}
\toprule
Lag Order ($p$) & Deterministic Branch Label & Historical Dummy & Rank ($r$) & Cointegrating Coefficient ($\hat{\theta}$) \\
\midrule
\midrule
1 & $d_0$ & $h_2$ & 1 & 0.91 \\
1 & $d_2$ & $h_2$ & 1 & 0.97 \\
...
\bottomrule
\end{tabular}
\caption*{Note: Retained means the specification passes the S2 statistical/system gates. Economic admissibility is imposed separately; the $d_3$ estimates and the above-unity $p2\_d0\_h2\_r1$ estimate are not treated as valid capacity benchmarks. Source: Author's elaboration.}
\end{table}

Figure~\ref{fig:s2-pooled-frontier} plots the specification frontier mapping system-level fit against parameter complexity for the trivariate VECM specifications. The results show that statistically retained parameters are constrained to a narrow region, while the economic-admissibility screen narrows the usable capacity benchmarks further. Bivariate systems do not pass the retained system-admissibility gates, and expanding the state vector beyond the trivariate specifications does not improve system-level identification, indicating that the trivariate system is the minimum retained specification in this design.
...
Heinz Kurz's (1986) classical choice-of-technique switching argument offers a mechanism through which changes in distribution, specifically the rate of exploitation $e_t$, alter the relative cost-effectiveness of different production techniques, prompting cost-minimizing producers to switch their technical methods and adjust their capital utilization, thereby endogenizing capacity. As producers respond to shifts in income distribution by changing their production techniques, the observed output-capital relation ($Y/K$) may become sensitive to distributional conditions rather than operating as a fixed technical law \citep{Kurz1986}. The S2 evidence is consistent with this mechanism: the bivariate system does not retain a stable cointegrating vector under the retained gates, while the trivariate system is statistically retained only when logged exploitation and historical shock controls enter the state vector. Within this design, capacity identification is therefore a distributionally conditioned capacity parameter rather than a pure technical engineering relation.

\subsection{Cross-Stage Econometric Synthesis}
\label{subsec:cross_stage_synthesis}

The empirical findings across the three estimation layers support a bounded critical-replication result. Stage S0 shows that the baseline capacity path is approximately recoverable, but not exactly reproducible under documented choices. Stage S1 scales this reconstruction into a non-empty statistically admissible ARDL space of disciplined non-uniqueness, where the long-run parameter varies along a Pareto fit--complexity frontier and some retained specifications fail economic admissibility. Stage S2 imposes joint multi-equation validation, showing that the bivariate output--capital relation is not self-sufficient and that restricted trivariate survival occurs under the retained criteria when logged exploitation and historical shock controls enter the long-run system.
...
S1 & Non-empty statistically admissible ARDL grid; non-unique retained benchmark & Model selection is driven by information-criterion choice \\
S2 Bivariate & 36 estimated, 0 system-admissible & Output--capital system is not self-sufficient \\
S2 Trivariate Rank One & 36 estimated, 6 statistically retained; 3 economically admissible & Restricted survival with logged exploitation and $h_2$ controls \\
S2 Trivariate Rank Two & 36 estimated, 0 system-admissible & No retained dual-vector system \\
\bottomrule
\end{tabular}
\caption*{Source: Author's elaboration.}
\end{table}

The magnitude of the estimated transformation elasticity ($\hat{\theta}$) dictates both the historical baseline of the capacity utilization path and its dynamic mean-reverting properties. Under Shaikh's methodology, utilization is constructed strictly as the residual distance between realized output and fitted capacity ($\ln \hat{\mu}_t = y_t - \hat{y}^p_t$). The estimated $\hat{\theta}$ therefore determines exactly how much of the smooth, long-run capital trend is subtracted from volatile current output. Lower parameter estimates force the residual to absorb structural drift, generating a utilization path characterized by long historical waves and dampened mean reversion. Conversely, higher parameter estimates aggressively strip the capital trend from the residual, resulting in a highly stationary trajectory that mirrors short-run business cycle volatility. 

Multi-equation vector estimation narrows this single-equation ambiguity by identifying the historical and institutional conditions under which structural stability is retained. The six statistically retained trivariate VECM specifications include the $h_2$ historical dummy vector, which accounts for the structural breaks of 1956, 1974, and 1980. This uniform survival pattern indicates that the outlier controls implemented in Shaikh's baseline operate as system-level conditions for the retained cointegrated space, rather than arbitrary statistical adjustments. Cointegration is retained only when these historically interpretable shock vectors are included in the long-run correlations between accumulation and production.

The non-self-sufficiency of the standalone output--capital relation is consistent with the dual macroeconomic role of capital accumulation. Because investment demand expands current output while the capital stock simultaneously builds productive capacity, the scalar parameter $\hat{\theta}$ is forced to mediate conflicting short-run and long-run forces. In a single-equation framework, this accounting entanglement is consistent with omitted distributional conditioning and parameter instability. The Stage S2 trivariate system indicates that the empirical measurement of capacity utilization is not fully captured as a purely technical engineering problem. Productive capacity is therefore a political-economy object, trackable in this design only when the estimation framework explicitly integrates the distributional conflict captured by the logged rate of exploitation.

\section{Conclusion}
\label{sec:conclusion}

This chapter has critically replicated Anwar Shaikh's capacity-utilization index by reconstructing the coefficient that generates the productive-capacity path. The baseline is approximately recoverable, but not exactly reproduced under documented choices, and the recovered coefficient is sensitive across statistically retained ARDL specifications. In the system estimates, the bivariate output--capital relation does not pass the retained system-admissibility gates. A restricted trivariate system survives under the retained criteria when logged exploitation and historical shock controls enter the long-run system. That survival does not make every retained estimate a valid capacity benchmark: some statistically retained specifications generate values at or above unity, including the S2 estimate $\hat{\theta}=1.19$, while others produce implausible negative or explosive magnitudes. These estimates are treated as failures of economic admissibility. The economically admissible retained benchmarks generally fall below unity, supporting a bounded interpretation in which capital accumulation can outpace capacity formation under the retained economic-admissibility criteria.

The evidence supports a bounded political-economy interpretation. In classical-Marxian profitability analysis, the general rate of profit is decomposed as the product of the profit share, capacity utilization, and the capacity-to-capital ratio. Under the economically admissible below-unity estimates, the capacity-to-capital ratio trends downward as capital accumulates, creating a channel through which falling capital productivity can weigh on profitability. The chapter does not estimate the demand components that may bound this tendency. A fuller account would need to examine whether military spending, household credit, or other autonomous demand components stabilize the capacity path across historical regimes. The system-level results show that the output--capital relation does not pass the retained system-admissibility gates without distributional and historical conditioning. That finding weakens the interpretation of the coefficient as an insulated technical parameter, but it does not by itself estimate the full causal mechanisms of class conflict or institutional intervention.

A key limitation of the present analysis is the treatment of the transformation elasticity $\theta$ as a time-invariant parameter. A fixed-parameter closure is necessary for this reconstruction, but the capacity-creating effect of capital accumulation may change across historical regimes of accumulation. Future research should therefore model $\theta$ as a dynamic parameter whose movement can be related to changes in technique, distribution, and institutional structure.
