...
This chapter has reexamined Anwar Shaikh's capacity utilization index, reconstructing the empirical and theoretical conditions required to identify the productive ceiling in the post-war U.S. economy. The replication shows that economically admissible retained specifications generally estimate the structural parameter governing the long-run output--capital relationship below unity ($\hat{\theta} < 1.0$). Some statistically retained specifications generate values at or above unity, including the S2 estimate $\hat{\theta}=1.19$, while others produce implausible negative or explosive magnitudes. These are treated as failures of economic admissibility rather than valid capacity benchmarks. The evidence rejects the standard Harrodian balanced-growth assumption ($\theta = 1.0$) only under the retained economic-admissibility criteria, indicating that the U.S. economy can be interpreted as operating under an unbalanced growth closure where capital accumulation outpaces capacity formation. In this regime, the system exhibits a structural tendency toward overaccumulation and stagnation, whose concrete historical stabilizers require direct treatment beyond the present chapter's empirical design.

The empirical results carry significant theoretical consequences for classical-Marxian profitability crisis models. In this tradition, the general rate of profit is decomposed as the product of the profit share, capacity utilization, and the capacity-to-capital ratio. Under the economically admissible below-unity estimates, the capacity-to-capital ratio exhibits a secular downward trend as capital accumulates. This falling capital productivity acts as a persistent downward drag on the rate of profit, reinforcing the Marxian tendency of the rate of profit to fall. The chapter does not estimate the demand components that may bound this tendency. A fuller account would need to examine whether military spending, household credit, or other autonomous demand components stabilize the capacity path across historical regimes. Our system-level analysis shows that omitting the distribution variable and historical shock vectors from the estimation yields spurious relationships, indicating that macroeconomic capacity utilization is not a neutral technical index but a variable shaped by class conflict and institutional interventions.
