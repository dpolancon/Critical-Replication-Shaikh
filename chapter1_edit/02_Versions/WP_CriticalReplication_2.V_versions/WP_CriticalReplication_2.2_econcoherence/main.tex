...
\begin{abstract}
Macroeconomic capacity utilization is conventionally constructed as a latent residual relative to an unobserved productive baseline. \citet{Shaikh2016} reconstructs this baseline from a fitted long-run output--capital level relationship, centering his empirical analysis on cyclical residual dynamics rather than the structural properties of the underlying coefficients. This paper critically replicates that strategy by reinterpreting the estimated coefficient as a candidate transformation elasticity ($\theta$) linking capital accumulation to productive-capacity formation. Within a three-stage replication design, Stage S0 establishes that a Shaikh-like capacity path is arithmetically reproducible from corporate national accounts, though the baseline parameter remains sensitive to unstated data and lag choices. Stage S1 opens an expansive single-equation autoregressive distributed lag (ARDL) specification grid across 500 estimated models, revealing an admissible parameter space characterized by a disciplined non-uniqueness where the dynamic mean-reversion properties of utilization shift continuously with information-criterion penalty schedules. Finally, Stage S2 evaluates the relation within a multi-equation vector error correction model (VECM) framework, showing that the standalone output--capital system fails the retained system-admissibility gates. Within the retained S2 grid, system stability appears only in a restricted, rank-one trivariate vector where the logged rate of exploitation explicitly anchors the cointegrating space. The paper provides evidence that the output--capital multiplier cannot be interpreted as an insulated technical production law; instead, valid capacity identification must place distribution and institutional shock vectors directly inside the structural estimation architecture.
\end{abstract}
...
The object of the chapter is narrow. It does not attempt a general history of output gaps or a complete theory of capitalist crisis. It asks whether Shaikh's output--capital capacity baseline can be reconstructed, whether the recovered long-run coefficient can be interpreted as a candidate transformation elasticity, whether that coefficient is unique across admissible single-equation ARDL specifications, and whether the relation remains admissible when estimated as a joint system.

The analytical weight placed on $\theta$ introduces an unbalanced growth closure, where the long-run transformation elasticity deviates from unity ($\theta \neq 1$). This formulation breaks with the Harrodian knife-edge assumption of a unitary capacity transformation that underlies post-Keynesian and neoclassical models. Under an unbalanced growth closure, long-run disequilibrium emerges as a persistent structural feature of accumulation, rather than a temporary disturbance. The evaluation of stationarity becomes a test of whether aggregate demand buffers stabilize the expansion or whether capital accumulation follows a regime-dependent trajectory.

To execute this critical replication, we apply the autoregressive distributed lag (ARDL) model, carefully considering the bounds testing procedure to determine the presence of a long-run relationship between output and capital, and vector error correction model (VECM) testing to examine the dynamic interactions among variables. The ARDL model is a single-equation approach that estimates the long-run relationship between output and capital. However, our results show that utilizing the companion data tables to \textit{Capitalism} does not yield exactly reproducible single-equation ARDL estimates. We recover approximate baseline results only after reconciling unstated data construction parameters and historical impulse controls. A grid search across 500 admissible ARDL specifications reveals that the output--capital coefficient lacks unique identification. The preferred parameter estimates shift with the information-criterion penalty schedules chosen by the researcher, exposing the fragility of single-equation cointegration claims.

In contrast, the VECM estimation provides a multi-equation framework that accounts for the interactions between variables, including the impact of historical events such as the 1956, 1974, and 1980 step-shift dummies on the cointegrating relationships. When we test the bivariate output--capital cointegration within a joint system framework, it fails the retained S2 admissibility gates across the estimated bivariate specifications, a result consistent with omitted distributional conditioning. In the retained S2 grid, admissible system stability appears only in a restricted, rank-one trivariate system where the logged rate of exploitation enters the state vector, reflecting the importance of distributional variables and historical shock vectors for the empirical results.

Our objective is to evaluate whether the parameter that constructs the productive ceiling can sustain the structural meaning required by an unbalanced growth interpretation. Since productive capacity constitutes the denominator of the utilization index, any drift in the estimated coefficient mechanically shifts the historical level, variance, and mean-reverting properties of the recovered series. The empirical findings reveal that while the baseline benchmark is arithmetically reproducible, it remains parameter-sensitive within the single-equation grid and lacks self-sufficiency at the system level. The identification of capitalist capacity paths cannot be resolved as a purely technical exercise, but requires a deeper understanding of the underlying macroeconomic structures.

AI-assisted anti-smoothing checks are treated only as a subordinate editing diagnostic in this chapter. They are useful for detecting repeated transitions, theatrical phrasing, and synthetic polish, but they do not govern the replication standard, the econometric design, or the political-economy interpretation.
...
To evaluate this single-equation parameter sensitivity, consider a key pedagogical counterfactual: the restricted balanced-growth model where the transformation elasticity is constrained to unity ($\theta = 1.0$). If a researcher forces this standard Harrodian assumption onto the US dataset, the capacity path is forced to expand exactly in step with the capital stock (imposing a constant capital productivity baseline). When we construct capacity utilization under this constraint, the resulting series drifts to economically absurd levels, either falling continuously toward zero in the long run or exceeding 120\% during periods of rapid expansion. This occurs because the economically retained estimates imply an unbalanced US output--capital relation ($\theta < 1.0$), meaning capacity grows more slowly than the capital stock. Imposing the $\theta=1$ counterfactual fails the cointegration admissibility gates, showing why estimating the unrestricted parameter is not just a technical choice, but a requirement for economic viability.
...
Figure~\ref{fig:s2-pooled-frontier} plots the specification frontier mapping system-level fit against parameter complexity for the trivariate VECM specifications. The results show that statistically retained parameters are constrained to a narrow region, while the economic-admissibility screen narrows the usable capacity benchmarks further. Bivariate systems fail to establish cointegration, and expanding the state vector beyond the trivariate specifications does not improve system-level identification, indicating that the trivariate system is the minimum retained specification in this design.
...
Multi-equation vector estimation resolves this single-equation ambiguity by exposing the historical and institutional conditions necessary for structural stability. The six statistically retained trivariate VECM specifications strictly require the inclusion of the $h_2$ historical dummy vector, which accounts for the structural breaks of 1956, 1974, and 1980. This uniform survival pattern indicates that the outlier controls implemented in Shaikh's baseline operate as system-level prerequisites for capturing a cointegrated space, rather than arbitrary statistical adjustments. Cointegration is retained only when these historically interpretable shock vectors stabilize the long-run correlations between accumulation and production.

The structural failure of the standalone output--capital relation stems from the dual macroeconomic role of capital accumulation. Because investment demand expands current output while the capital stock simultaneously builds productive capacity, the scalar parameter $\hat{\theta}$ is forced to mediate conflicting short-run and long-run forces. In a single-equation framework, this accounting entanglement creates system-level instability consistent with omitted distributional conditioning and parameter instability. The Stage S2 trivariate system provides evidence that the empirical measurement of capacity utilization cannot be resolved as a purely technical engineering problem. Productive capacity is therefore a political economy object, trackable in this design only when the estimation framework explicitly integrates the distributional conflict captured by the logged rate of exploitation.
