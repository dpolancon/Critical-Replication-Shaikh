...
The results are presented in Table~\ref{tab:s2_admissibility_outcomes}, which details the structural outcomes across the complete VECM specification space ($\mathcal{G}_{S2}$). The bivariate $r=1$ configuration yields zero admissible models, with VECM residuals that fail unit-root tests and remain nonstationary. In contrast, the trivariate $r=1$ framework yields six admissible specifications when conditioned on the logged rate of exploitation, $\ln e_t$. The trivariate rank-two permutations confirm this restriction, producing zero stable dual-vector systems. This indicates that cointegration is retained only within the rank-one trivariate system in the estimated S2 grid, alongside step-shift dummies, highlighting the importance of accounting for distributional variables in the analysis of the output-capital relationship.
...
Multi-equation vector estimation resolves this single-equation ambiguity by exposing the historical and institutional conditions necessary for structural stability. The six surviving trivariate VECM specifications strictly require the inclusion of the $h_2$ historical dummy vector, which accounts for the structural breaks of 1956, 1974, and 1980. This uniform survival pattern indicates that the outlier controls implemented in Shaikh's baseline operate as systemic prerequisites for capturing a cointegrated space, rather than arbitrary statistical adjustments. Cointegration is retained only when these institutional shock vectors stabilize the long-run correlations between accumulation and production.
...
This chapter has reexamined Anwar Shaikh's capacity utilization index, reconstructing the empirical and theoretical conditions required to identify the productive ceiling in the post-war U.S. economy. The replication shows that economically retained no-trend and preferred system specifications estimate the structural parameter governing the long-run output--capital relationship below unity ($\hat{\theta} < 1.0$), while some statistically retained trend specifications produce economically invalid values above unity or below zero. This finding rejects the standard Harrodian balanced-growth assumption ($\theta = 1.0$) only under the retained economic-admissibility criteria, indicating that the U.S. economy can be interpreted as operating under an unbalanced growth closure where capital accumulation outpaces capacity formation. In this regime, the system exhibits a structural tendency toward overaccumulation and stagnation, whose concrete historical stabilizers require direct treatment in a subsequent pass.
